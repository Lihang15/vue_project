      ...
      <el-form-item label="手机验证码" prop="verificationCode">
        <el-input v-model="loginForm.verificationCode">
          <template slot="append">
            ...
            >获取验证码</el-button>
            <div v-else>倒计时{{ loginForm.count }}s</div>
          </template>
        </el-input>
      </el-form-item>
...
export default {
  data() {
    ...
    var checkPhoneCode = (rules, value, callback) => {
      if (value == this.loginForm.contenttext) {
        alert("验证码对了", value, this.loginForm.contenttext);
        return callback();
      } else {
  ...
  methods: {
    ...
    getloginPhoneCode() {
      ...
      this.loginForm.contenttext = "1234";
      // 向手机号发送验证码传入的参数
      //   let phoneCode = {
      //     phonenum: this.loginForm.phone,
      //     contenttext:
      //       "您正在修改密码，验证码为:" + numCode + "，切勿将验证码泄露给他人。"
      //   };
      // 调用接口，向手机号发送验证码
    ...
    login() {
      this.$refs.loginFormRef.validate(valid => {
        if (valid) {
          console.log("开始登录嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻嘻");
        } else {
